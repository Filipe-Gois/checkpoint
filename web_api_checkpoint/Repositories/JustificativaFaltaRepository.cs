...
namespace web_api_checkpoint.Repositories
{
    public class JustificativaFaltaRepository : IJustificativaFaltaRepository
    {
        ...
        public async Task<bool> CadastrarJustificativa(CadastrarJustificativaFalta cadastrarJustificativaFalta)
        {
            ...
            List<Guid?> idGestores = ctx.SquadGestor.Include(x => x.Squad).Where(x => x.IdSquad == presencaBuscada.Estagiario!.IdSquad).Select(x => x.IdUsuario).ToList();
            ...
            Debug.WriteLine(justificativaFalta);
        ...
        public async Task<bool> ValidarJustificativa(ValidarJustificativaFalta validarJustificativaFalta)
        {
            ...
            Notificacao notificacao = new()
            {
                Descricao = $"Sua justificativa de falta para o dia {justificativaFaltaBuscada.Presenca!.CalendarioLetivo!.Data.ToString("dd/MM/yyyy")} foi {(validarJustificativaFalta.StatusValidacao == (int)StatusValidacaoEnum.Rejeitado ? "aprovada" : "reprovada")}.",
